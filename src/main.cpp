...
int main(int argc, char *argv[]){
  ...
  if (parsedArgs.threads < 0) {
    ...
    setThreadNum((long) parsedArgs.threads);
  ...
  // Read from files for LISTEN
  std::vector<std::unique_ptr<std::ifstream>> files;
  while (is || !files.empty()) {
    try {
      // A very standard REPL
      // REPL: Read-Evaluate-Print-Loop
      std::string queryStr;
      if(files.empty()) queryStr = extractQueryString(is);
      else queryStr = extractQueryString(*files.front());
      
      // LISTEN
      if(queryStr.find("LISTEN") == 0){
        std::string file = queryStr.substr(9, queryStr.size() - 11);
        if(exists_test(file)){
          files.push_back(std::make_unique<std::ifstream>(file));
          file = file.substr(file.rfind('/') + 1);
          std::cout << ++counter << "\nANSWER = ( listening from " << file << " )\n";
        }
        else
        {
