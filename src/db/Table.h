...
class Table {
  ...
  friend class DuplicateQueryResult;
public:
...
public:
  ...
  public:
    ...
    std::vector<ValueType> getValue() const {return it->datum;}
...
public:
  ...
  /** 
   * For "DeleteQuery.cpp"
   * Delete the records related to the keys
   * @param key
   */
  void del(const std::vector<Table::KeyType> &keys);

  /**
   *  Whether key_copy is in keymap
   *  @param key
   *  @return bool: True: not in keymap; False: in keymap
   */
  // bool copyExist(KeyType key);
  ...
  /**
   * Return whether the table has the key 
   * @param key
   * @return
   */
    bool hasKey(KeyType key){
      return (keyMap.find(key) != this->keyMap.end());
    }
