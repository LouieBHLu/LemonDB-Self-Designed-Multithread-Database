...
void Database::copyTable(const std::string &tableName, const std::string &copyName) {
  auto it = this->tables.find(tableName);
  if (it == this->tables.end()) {
    throw TableNameNotFound("Error when trying to copy table \"" + tableName +
                            "\". Table not found.");
  }
  auto newTable = std::make_unique<Table>(copyName, *(it->second));
  this->registerTable(std::move(newTable));
}
