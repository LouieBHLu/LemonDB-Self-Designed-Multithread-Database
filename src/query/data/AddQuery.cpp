...
void* add_per_thread(void* thread_id){
    ...
    if (result.second) {
      ...
      if(thread_num >= 2 && (int)table->size() > 20){
          ...
          for (long i = 0; i < thread_num; i++){
              if(pthread_create(&threads[i], NULL, add_per_thread, (void *)i) != 0) return make_unique<ErrorMsgResult>(qname, this->targetTable.c_str(), "Fail to create thread %d");
          }
